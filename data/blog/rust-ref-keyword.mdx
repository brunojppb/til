...
I'm currently building a native app at work with the Rust programming language
(using [tauri](https://tauri.app/) as my base framework) and I came across this
match pattern where I did not want to consume the value bound to my variable. In
Rust terms, I wanted to only get a reference to the inner value of a pattern.

This is where the [ref](https://doc.rust-lang.org/std/keyword.ref.html) keyword
is handy. It allow us to extract the inner value of a pattern by borrowing
instead of moving. Let's take a look at an example:
...
The compiler does a great job at warning you about that move, but it's an error,
so it does not let you move forward. To fix that, you can use `ref`:
...
[Mutex](https://doc.rust-lang.org/std/sync/struct.Mutex.html) and shared across
threads. The ref keyboard allowed me to borrow this shared value in a safe way.
